...
function buildMissionControlDecisions(
  ...
  if (mission.mission_status === "critical" || mission.mission_status === "at_risk") {
    drafts.push({
      ...
      reasoning: `Platform Control reports ${mission.top_threat} as the primary threat.`,
      expected_impact: 90,
      confidence_score: 85,
  ...
  for (const threat of mission.threats.slice(0, 3)) {
    drafts.push({
      ...
      reasoning: `Ranked ${threat.severity} threat from Platform Control aggregation.`,
  ...
  for (const accelerator of mission.accelerators.slice(0, 2)) {
    drafts.push({
      ...
      reasoning: "Platform Control identified this as a platform accelerator.",
