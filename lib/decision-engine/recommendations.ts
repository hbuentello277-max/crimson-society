...
function buildMissionControlDecisions(
  ...
  if (mission.mission_status === "critical" || mission.mission_status === "at_risk") {
    drafts.push({
      ...
      category: "risk",
      title: "Stabilize platform-critical signals",
      summary: `Platform status is ${mission.mission_status} with score ${mission.mission_score}.`,
      reasoning: `Platform Status reports ${mission.top_threat} as the primary threat.`,
      expected_impact: 90,
      confidence_score: 85,
  ...
  for (const threat of mission.threats.slice(0, 3)) {
    drafts.push({
      ...
      reasoning: `Ranked ${threat.severity} threat from Platform Status aggregation.`,
  ...
  for (const accelerator of mission.accelerators.slice(0, 2)) {
    drafts.push({
      ...
      reasoning: "Platform Status identified this as a platform accelerator.",
...
function buildReportDecisions(report: ReportContext, generatedAt: string): DecisionRecommendation[] {
  ...
  if (degraded.length > 0 || (report.mission.score ?? 100) < 65) {
    drafts.push({
      ...
      summary: `${degraded.length} workflow(s) below target; platform workflow score ${report.mission.score ?? "unknown"}.`,
