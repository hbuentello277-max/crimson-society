...
import {
  CorrelationRelatedRoutes,
  CorrelationSignalRow,
} from "@/components/nexus/correlations/CorrelationSignalRow";

const CATEGORY_LABELS: Record<CorrelationItem["category"], string> = {
  growth: "Growth",
  revenue: "Revenue",
...
export function CorrelationCard({ item }: { item: CorrelationItem }) {
  return (
    <article
    ...
    >
      ...
      <p className="mt-2 text-[10px] uppercase tracking-[0.16em] text-[#e87a82]">
        Nexus connected these signals
      </p>
      ...
      <div className="mt-4 space-y-2">
        {item.signals.map((signal) => (
          <CorrelationSignalRow key={`${item.id}-${signal.label}-${signal.source}`} signal={signal} />
        ))}
      </div>
      ...
      <div className="mt-4">
        <CorrelationRelatedRoutes routes={item.related_routes} />
      </div>
