  ...
  return (
    <article className="rounded-xl border border-white/10 bg-black/30 p-3 sm:p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <p className="break-words text-sm font-medium text-white">{objective.title}</p>
        <span className={`text-[10px] uppercase tracking-[0.14em] ${trackTone}`}>{trackLabel}</span>
      </div>
      <p className="mt-2 break-words text-sm leading-6 text-zinc-400">{objective.summary}</p>
      <p className="mt-2 break-words text-xs leading-5 text-zinc-500">{objective.recommendation}</p>
