...
import { CHAT_SOURCE_ROUTES } from "@/lib/chat/prompts";
...
export function ChatSources({ sources, relatedRoutes, confidence }: ChatSourcesProps) {
      ...
      {sources.length > 0 ? (
        <div className="min-w-0">
          ...
          <ul className="mt-1.5 flex flex-wrap gap-1.5">
            {sources.map((source) => {
              const href = CHAT_SOURCE_ROUTES[source];
              return (
                <li key={source}>
                  {href ? (
                    <Link
