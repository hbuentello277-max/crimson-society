...
import { formatWeeklyBriefingPlainText } from "@/lib/briefings/plain-text";
    ...
    <article className={`${CARD_CLASS} space-y-5`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.28em] text-[#e87a82]">Weekly Owner Briefing</p>
        ...
        >
          {copyState === "copied" ? "Copied" : copyState === "error" ? "Copy Failed" : "Copy Weekly Briefing"}
        </button>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <BriefingSectionBlock section={briefing.community_summary} />
...
export function weeklyBriefingPlainText(briefing: WeeklyOwnerBriefing) {
  return formatWeeklyBriefingPlainText(briefing);
}
