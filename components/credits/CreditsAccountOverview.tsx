  ...
  return (
    <section className="overflow-hidden rounded-[22px] border border-[#b4141e]/25 bg-gradient-to-br from-[#120608] via-[#0a0a0b] to-[#090909] p-4 shadow-[0_16px_50px_-36px_rgba(180,20,30,0.55)]">
      <div className="mb-3 flex items-center gap-2">
        ...
        <p className="text-[10px] uppercase tracking-[0.22em] text-[#e87a82]">{copy.account}</p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <p className="text-[10px] uppercase tracking-[0.24em] text-zinc-500">{copy.currentBalance}</p>
          <p className="mt-1 font-serif text-3xl text-white">
            {loading ? "—" : `${account.credits_balance}`}
            <span className="ml-1 text-sm font-normal text-zinc-500">{copy.credits}</span>
          </p>
          <p className="mt-1 text-xs text-zinc-500">
            {loading
              ? "—"
              : copy.rewardValue.replace(
                  "{value}",
                  formatCreditsRewardValueUsd(account.credits_balance),
                )}
          </p>
        </div>

        <div>
          <p className="text-[10px] uppercase tracking-[0.24em] text-zinc-500">{copy.monthlyEarned}</p>
      ...
      {!compact && (
        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-white/8 pt-4">
          <div className="rounded-xl border border-white/8 bg-black/20 px-3 py-2.5">
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">{copy.lifetimeEarned}</p>
            <p className="mt-1 text-lg font-medium text-zinc-200">
              {loading ? "—" : account.lifetime_credits_earned}
            </p>
          </div>
          <div className="rounded-xl border border-white/8 bg-black/20 px-3 py-2.5">
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">{copy.lifetimeSpent}</p>
            <p className="mt-1 text-lg font-medium text-zinc-200">
