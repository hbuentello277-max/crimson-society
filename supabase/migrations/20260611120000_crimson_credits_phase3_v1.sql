...
-- Meet earn triggers
-- ---------------------------------------------------------------------------
create or replace function public.trg_award_meet_attend_credit()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  v_host_id uuid;
begin
  select r.host_id into v_host_id
  from public.rides r
  ...
  perform public.award_crimson_credits(
    ...
    'meet_attended',
    'Attended a meet',
    'meet_attend:' || new.user_id::text || ':' || new.ride_id::text,
...
exception
  when others then
    raise warning 'meet attend credit failed: %', sqlerrm;
    return new;
end;
$$;

drop trigger if exists award_meet_attend_credit_after_insert on public.ride_attendees;
create trigger award_meet_attend_credit_after_insert
after insert on public.ride_attendees
for each row execute function public.trg_award_meet_attend_credit();

create or replace function public.trg_award_meet_host_credit()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  perform public.award_crimson_credits(
    ...
    'meet_hosted',
    'Hosted a meet',
    'meet_host:' || new.host_id::text || ':' || new.id::text,
...
exception
  when others then
    raise warning 'meet host credit failed: %', sqlerrm;
    return new;
end;
$$;

drop trigger if exists award_meet_host_credit_after_insert on public.rides;
create trigger award_meet_host_credit_after_insert
after insert on public.rides
for each row execute function public.trg_award_meet_host_credit();
