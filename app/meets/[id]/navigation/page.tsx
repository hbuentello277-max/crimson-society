...
export default function MeetNavigationPage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      const sessionMeet = sessionPayloadToActiveRide(readActiveMeetSession());
      if (sessionMeet && (!meetId || sessionMeet.id === meetId)) {
        if (!cancelled) {
          setActiveRide(sessionMeet);
          setLoaded(true);
        }
        return;
      }
      ...
      const fromDb = await bootstrapActiveMeetFromDb(userId, meetId);
      if (cancelled) return;
