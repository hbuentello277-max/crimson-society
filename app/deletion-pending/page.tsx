...
export default function DeletionPendingPage() {
  ...
  const cancelDeletion = async () => {
    ...
    const confirmed = window.confirm("Cancel your account deletion request and restore full access?");
    if (!confirmed) return;
    ...
    try {
      ...
      setMessage(result?.message || "Deletion request canceled.");
      await loadRequest();
    } catch (error) {
      ...
      <main className="min-h-screen bg-[#050505] px-5 pt-[calc(env(safe-area-inset-top)+2rem)] text-white">
        <div className="mx-auto max-w-lg">
          ...
          <p className="mt-4 text-sm leading-6 text-zinc-400">
            If you requested account deletion, sign in here to view status or cancel while your request
            is still pending.
          </p>
          <Link
            href="/login"
          ...
          <Link
            ...
            className="mt-4 block text-xs uppercase tracking-[0.16em] text-zinc-500 hover:text-[#e87a82]"
          >
            Read how deletion works
          </Link>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[#050505] px-5 pb-28 pt-[calc(env(safe-area-inset-top)+2rem)] text-white">
      <div className="mx-auto max-w-lg">
        ...
        <p className="mt-4 text-sm leading-6 text-zinc-400">
          Your account is in <span className="text-zinc-200">deletion_pending</span> status. App access
          is limited to this screen, cancellation, and account-deletion information until an admin
          approves or you cancel.
        </p>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          {loadingRequest ? (
            ...
            <>
              ...
              <p className="mt-2 text-sm text-[#e87a82]">{deletionStatusLabel(request.status)}</p>
              <p className="mt-3 text-sm leading-6 text-zinc-400">
                {deletionStatusUserMessage(request)}
              </p>
            </>
          ) : (
            <p className="text-sm text-zinc-500">No deletion request record found.</p>
          )}
        </section>
        ...
        <div className="mt-6 grid gap-2">
          {request?.status === "pending" && (
            <button
              ...
              className="rounded-xl border border-white/10 px-4 py-3 text-left text-xs uppercase tracking-[0.16em] text-zinc-200 transition hover:border-[#b4141e]/50 hover:text-[#e87a82] disabled:opacity-60"
            >
              {busy ? "Canceling…" : "Cancel deletion request"}
            </button>
          )}

          <Link
            href="/account-deletion"
            className="rounded-xl border border-white/10 px-4 py-3 text-xs uppercase tracking-[0.16em] text-zinc-400 transition hover:border-[#b4141e]/50 hover:text-[#e87a82]"
          >
          ...
          <button
            ...
            className="rounded-xl border border-white/10 px-4 py-3 text-left text-xs uppercase tracking-[0.16em] text-zinc-500 transition hover:text-zinc-300"
          >
            Sign out
          </button>
        </div>
        ...
        {request && isOpenDeletionStatus(request.status) ? (
          <p className="mt-6 text-xs leading-5 text-zinc-600">
            After admin approval, your Blackcard subscription will be canceled, personal data removed,
            and your sign-in account deleted. Moderation records may be retained as described in our
            Privacy Policy.
          </p>
        ) : null}
      </div>
